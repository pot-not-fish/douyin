...
func (video *Video) Publish() error {
	...
	dal.RedisDB.LPush("VideoId", video.ID)

	len, err := dal.RedisDB.LLen("VideoId").Result()
	if err != nil {
		return err
	}

	if len > 1000 {
		dal.RedisDB.RPop("VideoId")
	}
...
func VideoSubscribe(offset int64, limit int64) ([]Video, int64, error) {
	...
	str_videoid_list, err := dal.RedisDB.LRange("VideoId", offset, limit-1).Result()
	if err != nil {
		return nil, 0, err
	}
...
func IncCommentCache(video_id int64) error {
	...
	if err = dal.RedisDB.HIncrBy("video_comment_count", fmt.Sprintf("%d", video_id), 1).Err(); err != nil {
		return err
	}
