...
func (f *Favorite) CreateFavoriteCache() error {
	...
	ok, err := redisDB.SIsMember("favorite", fmt.Sprintf("%d-%d", f.UserID, f.VideoID)).Result()
	if err != nil {
		return err
	}
	if ok {
	...
	if err = redisDB.SAdd("favorite", fmt.Sprintf("%d-%d", f.UserID, f.VideoID)).Err(); err != nil {
		return err
	}
...
func (f *Favorite) DeleteFavoriteCache() error {
	...
	if err = redisDB.SRem("favorite", fmt.Sprintf("%d-%d", f.UserID, f.VideoID)).Err(); err != nil {
		return err
	}
...
func IsFavorite(user_id int64, video_id_list []int64) ([]bool, error) {
	...
	_, err = redisDB.Pipelined(func(p redis.Pipeliner) error {
		var is_favorite bool
		for _, v := range video_id_list {
			ok, err := redisDB.SIsMember("favorite", fmt.Sprintf("%d-%d", user_id, v)).Result()
			if err != nil {
				return err
			}
			is_favorite = ok
